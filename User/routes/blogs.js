...
router.get('/blognew', (request, response) => {
  response.render('blognew');
});
...
router.get('/:slug', async (request, response) => {
  ...
  if (blog) {
    response.render('blogshow', { blog: blog });
  } else {
    response.redirect('/');
  }
});
...
router.get('/blogedit/:id', async (request, response) => {
  let blog = await Blog.findById(request.params.id);
  response.render('blogedit', { blog: blog });
});
...
router.put('/:id', async (request, response) => {
  ...
  try {
    ...
    console.log(error);
    response.redirect(`/seblogs/blogedit/${blog.id}`, { blog: blog });
  }
});
